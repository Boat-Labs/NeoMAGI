...
async def test_skill_store_persists_jsonb_fields_with_asyncpg(
    ...
    spec = SkillSpec(
        ...
        delta=(
            "Summarize the user's goal first.",
            "List exactly three execution steps.",
    ...
    updated_evidence = SkillEvidence(
        source="human-taught",
        success_count=1,
        failure_count=1,
    ...
    proposal = GrowthProposal(
        ...
        payload={
            ...
            "skill_evidence": updated_evidence.model_dump(),
        },
        proposed_by="user",
    )
    ...
    assert record.proposal["payload"]["skill_evidence"]["positive_patterns"] == list(
        updated_evidence.positive_patterns
    )
    ...
    assert evaluated.eval_result["checks"][0]["name"] == "schema_validity"
