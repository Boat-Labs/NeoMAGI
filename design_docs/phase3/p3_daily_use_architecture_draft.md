...
- Claude provider 初期可先支持普通 chat / streaming；tool / image / structured output 由 P3-M1 具体计划按 capability 分阶段打开。
- 当前 Gemini 继续通过 OpenAI-compatible endpoint 使用 `OpenAICompatModelClient`；`GeminiNativeModelClient` 只是未来候选，不进入 P3 默认范围。
- tool calling、image support、max context 等能力必须显式标注 capability。
- provider / model 不支持当前任务所需能力时，必须显式降级或拒绝，不允许静默失败。
- Anthropic Messages API 与 OpenAI Chat Completions 的 tool call 消息格式不同；message / tool / image / streaming mapping 是 provider adapter 的核心职责，不进入 Gateway / AgentLoop。
...
ModelClient provider adapter
  OpenAICompatModelClient (OpenAI and current Gemini OpenAI-compatible endpoint)
...
- 不把 LiteLLM / OpenAI-compatible proxy 作为 P3-M1 Claude 主路径；它可以作为未来扩展或 smoke path 评估。
...
P3-M1 `memory_search` query expansion 优先采用 PG-native、corpus-grounded 路线，不在每次 search hot path 默认调用 LLM 扩关键词。
...
3. Corpus-grounded topic inventory：仍低召回且 query 是宽泛类别时，作为 `memory_search` 的 internal fallback，从用户自己的 memory corpus 生成 topic / keyword inventory，例如基于 `ts_stat` / `search_vector` 统计实际出现的主题词，再返回候选主题或用这些真实词做二次搜索。
4. Per-entry retrieval keywords / tags：P3-M1 可利用现有 `memory_entries.tags` 存放 indexer 阶段提取的 top-N 高信号关键词；这些 tags 是 retrieval projection，不是 memory truth，也不是完整 taxonomy。
...
- P3-M1 不新增独立 `memory_topics` tool；只有 daily cases 证明用户需要主动浏览记忆主题时再提升为单独 tool。
- `pg_trgm` 可作为 typo / 近似字符串匹配的 optional enhancement，不作为 P3-M1 主方案。
- 不把“技术”“投资”“健康”等宽泛词自动硬映射到一套全局 taxonomy；宽泛查询应优先返回用户记忆库中的实际候选主题。

Embedding 可以预留，但暂不作为 P3-M1 必选项：
...
- `status` 与 `integrity_status` 是两个维度：`status` 表示逻辑生命周期，`integrity_status` 表示最近一次物理文件验证结果。
- `active + missing` 表示 metadata 仍声明 artifact 存在但文件丢失，doctor 应报告 drift，不能静默改成 deleted。
- `active + modified` 表示文件 hash 与 baseline 不一致，doctor 应报告 drift，接受当前 hash 必须走显式命令。
- `deleted + missing` 通常是可接受 tombstone；`archived + missing` 是否报警由后续归档策略决定。
- P3 daily artifacts 范围比已有 builder work memory 更宽，使用独立 `artifacts` 表 / store；不复用 builder artifact 作为总表。
- 既有 builder artifact 不在 P3-M1 强制迁移；未来可登记进 `artifacts` 表并以 `metadata.origin = "builder"` 标记，ADR 0055 的 builder work memory 语义不被取代。
...
- `stdout_tail` / `stderr_tail` 入库前必须截断；默认每个 stream 保留最后 16 KiB，UTF-8 safe truncate。
- 完整 stdout / stderr 写入 run log 文件或 log artifact，前端默认展示 tail，需要时打开完整日志。
- tail 入库前做基本 secret redaction，至少覆盖常见 token / API key / credential pattern。
...
- P3-M1 需要打通受控写入路径：`memory_append` / memory writer 可接收经过校验的 `artifact_ids`，写入 ledger metadata。
