...
- 当前仓库仍把 `beads` 远端同步写成 `just beads-pull` / `just beads-push`，底层通过 [`.beads/dolt/NeoMAGI`](../.beads/dolt/NeoMAGI) 执行 `dolt pull origin main` / `dolt push origin main`。
...
- 采用“Git 跟踪的 JSONL 备份”作为项目级 `beads` 的远端恢复路径：
  - 用 `bd backup --force` 生成并刷新 [`.beads/backup/`](../.beads/backup) 下的 JSONL 备份。
  - 通过当前项目仓库的普通 `git add / commit / push` 同步这些备份文件。
- 明确禁止把 [`.beads/dolt/`](../.beads/dolt) 或其他 Dolt 内部目录直接纳入主仓库 Git 版本控制。
...
- 方案 B：把 [`.beads/dolt/`](../.beads/dolt) 整体纳入项目 Git，当作“网盘备份”。
...
- [`justfile`](../justfile) 后续应从 `beads-pull` / `beads-push` 迁移到更贴近实际行为的命令，例如 `beads-backup`、`beads-backup-status`、`beads-restore-dry-run`。
- [`AGENTS.md`](../AGENTS.md) 与 [`CLAUDE.md`](../CLAUDE.md) 的 session completion 规则需要更新：
  - 代码 / 文档改动仍走普通 `git push`
  - 只有在本轮修改了 beads issue 数据时，才额外刷新并提交 `.beads/backup/*`
- [`.beads/config.yaml`](../.beads/config.yaml) 中当前的 `sync.git-remote` 不再应被视为日常依赖路径；后续可选择删除、注释化，或仅作为历史兼容配置保留。
